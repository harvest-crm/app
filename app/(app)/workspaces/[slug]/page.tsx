  ...
  return (
    <div className="p-8">
      ...
      {workspace.stages.length > 0 && (
        <div className="mt-8">
          <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-[#3D5775]">Stages</h2>
          <div className="flex flex-wrap gap-2">
            {workspace.stages.map((stage) => (
              <div key={stage.id} className="rounded-md border bg-white px-3 py-1.5 text-sm">
                {stage.name}
                {stage.isTerminal && (
                  <span className="ml-2 text-xs text-[#3D5775]">({stage.terminalOutcome})</span>
                )}
              </div>
            ))}
          </div>
