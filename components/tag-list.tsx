...
function TagRow({ tag, onSaved }: { tag: TagWithCount; onSaved?: () => void }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-md border bg-white px-3 py-2">
      ...
      <span className="text-xs text-slate-400">{tag._count.contactTags} contact{tag._count.contactTags !== 1 ? "s" : ""}</span>
...
function NewTagRow({ onCreated }: { onCreated?: () => void }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-md border-2 border-dashed border-slate-300 bg-slate-50 px-3 py-2">
      <span
        className="h-3 w-3 shrink-0 rounded-full"
        style={{ backgroundColor: color }}
      />
