...
const inputCls =
  "w-full rounded-md border border-[#E8DFC8] px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#1F8A8A]";
...
function FieldInput({
  ...
  const label = (
    <div className="flex items-center gap-1 mb-1">
      <span className="text-xs font-medium text-[#3D5775]">{def.fieldLabel}</span>
        ...
        <div className="space-y-1.5">
          {def.options.map((opt) => (
            <label key={opt} className="flex cursor-pointer items-center gap-2">
              ...
              />
              <span className="text-sm text-[#3D5775]">{opt}</span>
            </label>
          ))}
        </div>
      </div>
    ...
    <div className="space-y-6">
      {groups.map((g) => (
        <div key={g.workspaceId}>
          {groups.length > 1 && (
            <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-[#3D5775]">
              {g.workspaceName}
            </p>
          )}
          <div className="grid grid-cols-2 gap-4">
