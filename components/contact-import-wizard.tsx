...
export function ContactImportWizard({ workspaces, allTags }: Props) {
    ...
    <div className="space-y-6">
      ...
      <div
        ...
        className={cn(
          ...
          isDragOver ? "border-[#D0E5E2] bg-[#E2F0EE]" : "border-[#E8DFC8] bg-[#F5EFE0] hover:border-[#E8DFC8] hover:bg-white",
          parsing && "pointer-events-none opacity-60",
        )}
      >
        <Upload className="h-10 w-10 text-[#3D5775]" />
        <div>
          <p className="text-base font-medium text-[#3D5775]">{parsing ? "Parsing…" : "Drop your CSV here"}</p>
          <p className="text-sm text-[#3D5775]">or click to browse · .csv only · max 10 MB</p>
        </div>
      </div>
  ...
  if (step === 2) return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-[#3D5775]">{total} rows detected. Map each column to a contact field.</p>

      <div className="divide-y rounded-xl border bg-white">
        {headers.map((h) => (
          <div key={h} className="flex items-center gap-4 px-4 py-3">
            <span className="w-48 shrink-0 truncate text-sm font-medium text-[#3D5775]">{h}</span>
            <span className="text-[#3D5775]">→</span>
            <select
              value={mappings[h] ?? "skip"}
              onChange={(e) => setMappings((prev) => ({ ...prev, [h]: e.target.value }))}
              className="min-w-0 flex-1 rounded-md border border-[#E8DFC8] bg-white px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#1F8A8A]"
            >
      ...
      {!hasRequiredMapping && (
        <p className="flex items-center gap-1.5 text-sm text-[#1F8A8A]">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <button onClick={() => setStep(1)} className="rounded-md border border-[#E8DFC8] px-4 py-2 text-sm text-[#3D5775] hover:bg-[#E2F0EE]">Back</button>
          <button onClick={() => setStep(3)} disabled={!hasRequiredMapping}
            className="rounded-md bg-[#1F8A8A] px-4 py-2 text-sm font-semibold text-white hover:bg-[#1A7575] disabled:opacity-40">
            Continue
          </button>
        </div>
      </div>
  ...
  if (step === 3) return (
    <div className="space-y-6">
      ...
      <div className="space-y-4 rounded-xl border bg-white p-6">
        {workspaces.length > 0 && (
          <div className="space-y-1.5">
            <label className="block text-xs font-semibold uppercase tracking-wide text-[#3D5775]">Add to Workspace</label>
        ...
        <div className="space-y-1.5">
          <label className="block text-xs font-semibold uppercase tracking-wide text-[#3D5775]">Override Source</label>
        ...
        <div className="space-y-1.5">
          <label className="block text-xs font-semibold uppercase tracking-wide text-[#3D5775]">Default Temperature</label>
        ...
        {allTags.length > 0 && (
          <div className="space-y-2">
            <label className="block text-xs font-semibold uppercase tracking-wide text-[#3D5775]">Apply Tags to All</label>
      ...
      <div className="flex items-center justify-between">
        {cancelBtn}
        <div className="flex gap-3">
          <button onClick={() => setStep(2)} className="rounded-md border border-[#E8DFC8] px-4 py-2 text-sm text-[#3D5775] hover:bg-[#E2F0EE]">Back</button>
          <button onClick={enterPreview} className="rounded-md bg-[#1F8A8A] px-4 py-2 text-sm font-semibold text-white hover:bg-[#1A7575]">
            Preview import
          </button>
        </div>
      </div>
  ...
  if (step === 4) return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border bg-white p-4">
        ...
        <p className="mt-1 text-sm font-semibold text-[#0F2540]">
        ...
        <div className="overflow-x-auto rounded-xl border bg-white">
          <table className="w-full text-sm">
            <thead className="border-b bg-[#F5EFE0]">
              <tr>
                {previewCols.map((c) => (
                  <th key={c.value} className="px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide text-[#3D5775]">{c.label}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y">
              {previewRows.map((row, i) => (
                <tr key={i} className="hover:bg-[#E2F0EE]">
                  {previewCols.map((c) => (
                    <td key={c.value} className="max-w-[180px] truncate px-3 py-2 text-[#3D5775]">
          ...
          {total > 10 && <p className="px-3 py-2 text-xs text-[#3D5775]">Showing first 10 of {total} rows</p>}
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        {cancelBtn}
        <div className="flex gap-3">
          <button onClick={() => setStep(3)} className="rounded-md border border-[#E8DFC8] px-4 py-2 text-sm text-[#3D5775] hover:bg-[#E2F0EE]">Back</button>
          <button
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border bg-white p-6 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold text-[#0F2540]">Import complete</p>
            <p className="text-sm text-[#3D5775]">
        ...
        <div className="flex gap-3 pt-2">
          <Link href="/contacts" className="rounded-md bg-[#1F8A8A] px-4 py-2 text-sm font-semibold text-white hover:bg-[#1A7575]">
            View contacts
          </Link>
          <button onClick={() => { setStep(1); setImportResult(null); setCsvString(""); setHeaders([]); setAllRows([]); }}
            className="rounded-md border border-[#E8DFC8] px-4 py-2 text-sm text-[#3D5775] hover:bg-[#E2F0EE]">
            Import another file
          </button>
        </div>
      </div>
        ...
        return (
          <div key={n} className="flex items-center gap-2">
            <div className={cn(
              ...
              current ? "bg-[#1F8A8A] text-white"   : "bg-[#E2F0EE] text-[#3D5775]",
            )}>
              {done ? <Check className="h-3.5 w-3.5" /> : n}
            </div>
            <span className={cn("text-sm", current ? "font-semibold text-[#0F2540]" : "text-[#3D5775]")}>
              {label}
            </span>
            {i < labels.length - 1 && <span className="text-[#3D5775]">·</span>}
          </div>
        );
      })}
    </div>
